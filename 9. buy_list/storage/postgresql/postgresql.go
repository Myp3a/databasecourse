...
func (c *Connection) CreateUser(nickname string, name string, user_id int64, chat_id int64) bool {
	sqlStatement := `INSERT INTO users  (telegram_user_id, telegram_chat_id) VALUES ($1, $2)`
	_, err := c.conn.Exec(sqlStatement, user_id, chat_id)
...
func (c *Connection) StatusTime(user_id int64, chat_id int64, from time.Time, to time.Time) ([]product.Product, int, error) {
	used_prod := make([]product.Product, 0)
	trash_prod := 0
	var (
		name        string
